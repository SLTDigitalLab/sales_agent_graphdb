...
function Chat() {
  ...
  const handleMessageClick = (e) => {
    ...
    if (link && link.dataset.productName) {
      e.preventDefault();
      console.log("Interceptor: Switching to product", link.dataset.productName);
      fetchProductDetails(link.dataset.productName);
    }
  };
  ...
  const requestClearHistory = () => setShowClearConfirm(true);
  ...
  const handleOrderSuccess = (requestId) => {
    setInlineFormStates(prev => ({
      ...prev,
      [requestId]: { ...prev[requestId], submissionStatus: 'success' }
    }));
  };

  const InlineOrderForm = ({ requestId, initialMessage, savedState, onOrderSuccess, onSubmitError, prefillProduct }) => {
    const [items, setItems] = useState([{ productId: savedState?.items?.[0]?.productId || prefillProduct || '', quantity: savedState?.items?.[0]?.quantity || 1 }]);
    ...
    const addProductLine = () => setItems(prevItems => [...prevItems, { productId: '', quantity: 1 }]);

    const removeProductLine = (index) => {
      if (items.length > 1) setItems(prevItems => prevItems.filter((_, i) => i !== index));
    };
    ...
    const handleSubmit = async () => {
      ...
      if (items.some(item => !item.productId.trim())) { setFormError('Please select a product for all lines.'); return; }
      if (!customerDetails.name.trim() || !customerDetails.email.trim() || !customerDetails.phone.trim()) { setFormError('Please fill in your name, email, and phone number.'); return; }
      ...
      <div className="mt-3 space-y-4">
        ...
        {formError && (
          <div className="bg-red-50 border border-red-200 rounded-xl px-3 py-2">
            <div className="flex items-center gap-2"><AlertCircle className="w-4 h-4 text-red-600" /><p className="text-xs text-red-800 font-medium">{formError}</p></div>
          </div>
        )}
          ...
          {loadingProducts ? (
            ...
            items.map((item, index) => (
              <div key={index} className="flex gap-1 mb-2">
                  ...
                  {(() => {
                    ...
                    return Object.entries(categories).map(([category, prods]) => (
                      <optgroup key={category} label={category}>
                        {prods.map(p => (
                          <option key={`${p.sku}-${p.name}`} value={p.name}>
                ...
                />
                {items.length > 1 && (
                  <button type="button" onClick={() => removeProductLine(index)} className="p-1 text-red-600 hover:bg-red-50 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed" disabled={isSubmitting}><X className="w-3 h-3" /></button>
                )}
              </div>
            ))
          )}
          {!loadingProducts && (
            <button type="button" onClick={addProductLine} className="flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 font-medium mt-1 disabled:opacity-50 disabled:cursor-not-allowed" disabled={isSubmitting}><Plus className="w-3 h-3" /> Add Item</button>
          )}
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-2">
            <input type="text" placeholder="Name *" value={customerDetails.name} onChange={(e) => handleCustomerDetailChange('name', e.target.value)} className="w-full px-2 py-1 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500/20 focus:border-blue-500" disabled={isSubmitting} />
            <input type="email" placeholder="Email *" value={customerDetails.email} onChange={(e) => handleCustomerDetailChange('email', e.target.value)} className="w-full px-2 py-1 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500/20 focus:border-blue-500" disabled={isSubmitting} />
            <input type="tel" placeholder="Phone * (10 digits)" value={customerDetails.phone} onChange={(e) => handleCustomerDetailChange('phone', e.target.value.replace(/\D/g, ''))} maxLength="10" className="w-full px-2 py-1 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500/20 focus:border-blue-500" disabled={isSubmitting} />
            <textarea placeholder="Address (optional)" value={customerDetails.address} onChange={(e) => handleCustomerDetailChange('address', e.target.value)} rows="1" className="w-full px-2 py-1 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500/20 focus:border-blue-500 resize-none" disabled={isSubmitting}></textarea>
            <textarea placeholder="Notes (optional)" value={customerDetails.notes} onChange={(e) => handleCustomerDetailChange('notes', e.target.value)} rows="1" className="w-full px-2 py-1 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500/20 focus:border-blue-500 resize-none" disabled={isSubmitting}></textarea>
          </div>
        </div>

        <button onClick={handleSubmit} disabled={isSubmitting} className="mt-3 px-3 py-1.5 bg-gradient-to-br from-blue-500 to-indigo-600 text-white text-xs font-medium rounded-lg hover:from-blue-600 hover:to-indigo-700 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1">
          {isSubmitting ? (<><Loader2 className="w-3 h-3 animate-spin" /> Submitting...</>) : 'Submit Order'}
        </button>
      </div>
    );
  };
  ...
  const formatMessageContent = (content) => {
    ...
    formatted = formatted.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match, text, url) => {
      return `<a href="${url}" data-product-name="${text}" class="product-link text-blue-600 hover:text-blue-800 underline transition-colors font-medium cursor-pointer">${text}</a>`;
    });
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-slate-50 to-slate-100 overflow-hidden">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 transform scale-100 animate-in zoom-in-95 duration-200 border border-slate-100">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center flex-shrink-0"><Trash2 className="w-5 h-5 text-slate-600" /></div>
              <div><h3 className="text-lg font-bold text-slate-800">Clear History?</h3></div>
            </div>
            <p className="text-slate-600 mb-6 leading-relaxed">Are you sure you want to delete all chat history? This action cannot be undone.</p>
            <div className="flex gap-3 justify-end">
              <button onClick={() => setShowClearConfirm(false)} className="px-4 py-2 text-slate-600 font-medium hover:bg-slate-50 hover:text-slate-900 rounded-xl transition-colors border border-transparent hover:border-slate-200">Cancel</button>
              <button onClick={confirmClearHistory} className="px-4 py-2 bg-slate-500 text-white font-medium rounded-xl hover:bg-slate-700 transition-colors shadow-sm flex items-center gap-2"><Trash2 className="w-4 h-4" />Yes, Clear Chat</button>
            </div>
          </div>
        </div>
      )}

      <header className="bg-white border-b border-slate-200 shadow-sm relative z-10 flex-shrink-0">
        <div className="w-full px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center shadow-sm"><Bot className="w-6 h-6 text-white" /></div>
            <div>
      ...
      <div className="flex flex-1 overflow-hidden relative">
            ...
            <div className="max-w-5xl mx-auto px-4 py-6">
              {messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center py-12">
                  ...
                  <p className="text-slate-600 mb-8 max-w-md leading-relaxed">Ask me anything about SLT's products, services, or company information.</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-2xl w-full">
                    <button onClick={() => setInputValue("I want to order a product")} className="p-4 bg-white border border-slate-200 rounded-xl hover:border-blue-400 hover:shadow-md transition-all text-left group">
                    ...
                    <button onClick={() => setInputValue("What are available product categories?")} className="p-4 bg-white border border-slate-200 rounded-xl hover:border-blue-400 hover:shadow-md transition-all text-left group">
                    ...
                    <button onClick={() => setInputValue("Tell me about SLT's services")} className="p-4 bg-white border border-slate-200 rounded-xl hover:border-blue-400 hover:shadow-md transition-all text-left group">
                    ...
                    <button onClick={() => setInputValue("What security cameras do you offer?")} className="p-4 bg-white border border-slate-200 rounded-xl hover:border-blue-400 hover:shadow-md transition-all text-left group">
                    ...
                    return (
                      <div key={message.id} className={`flex gap-3 ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        <div className={`flex flex-col max-w-3xl ${message.type === 'user' ? 'items-end' : 'items-start'}`}>
                          ...
                          <div onClick={handleMessageClick} className={`rounded-2xl px-5 py-3.5 ${message.type === 'user' ? 'bg-gradient-to-br from-blue-500 to-indigo-600 text-white shadow-md' : message.type === 'error' ? 'bg-red-50 text-red-800 border border-red-200' : 'bg-white text-slate-800 shadow-sm border border-slate-200'}`}>
                            <>
                        ...
                        {message.type === 'user' && (<div className="w-8 h-8 bg-gradient-to-br from-slate-600 to-slate-700 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm"><User className="w-5 h-5 text-white" /></div>)}
                      </div>
                    );
                  })}
                  {isLoading && (
                    <div className="flex gap-3 justify-start animate-pulse">
                      <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center flex-shrink-0 opacity-80"><Bot className="w-5 h-5 text-white" /></div>
                      <div className="bg-white rounded-2xl px-4 py-3 shadow-sm border border-slate-200"><div className="flex items-center gap-2"><Loader2 className="w-4 h-4 text-blue-500 animate-spin" /><span className="text-sm text-slate-600 font-medium">Thinking...</span></div></div>
                    </div>
                  )}
          ...
          <div className="border-t border-slate-200 bg-white">
            <div className="max-w-5xl mx-auto px-4 py-4">
              <form onSubmit={sendMessage} className="flex gap-3">
                <input ref={inputRef} type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="Ask about products, prices, or company information..." disabled={isLoading} className="flex-1 px-5 py-3.5 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 disabled:bg-slate-50 disabled:cursor-not-allowed text-slate-800 placeholder-slate-400 transition-all shadow-sm" />
                <button type="submit" disabled={!inputValue.trim() || isLoading} className="px-6 py-3.5 bg-gradient-to-br from-blue-500 to-indigo-600 text-white rounded-xl hover:from-blue-600 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2 font-medium shadow-md shadow-blue-500/20">
                  {isLoading ? (<Loader2 className="w-5 h-5 animate-spin" />) : (<><Send className="w-5 h-5" /><span className="hidden sm:inline">Send</span></>)}
                </button>
              </form>
        ...
        {canvasData && (
          <div className="hidden lg:block w-[400px] border-l border-slate-200 bg-white shadow-lg relative z-20 animate-in slide-in-from-right duration-300">
            <ProductCanvas product={canvasData} isLoading={canvasLoading} />
          </div>
        )}
