...
def run_linkedin_scraper(li_url: str, max_posts: int = 5) -> str:
    ...
    try:
        ...
        logger.error(f"Error in LinkedIn Scraper: {e}", exc_info=True)
...
def run_general_scraping():
    ...
    if website_urls:
        logger.info(f"Starting Website Scraper for {len(website_urls)} URLs")
        try:
            ws = WebsiteScraper(website_urls)
            ws.scrape()
            results["website"] = f"success ({len(website_urls)} sites scraped)"
        except Exception as e:
            logger.error(f"Error in Website Scraper: {e}", exc_info=True)
    ...
    if fb_url:
        ...
        try:
            ...
            logger.error(f"Error in Facebook Scraper: {e}", exc_info=True)
    ...
    if tiktok_url:
        ...
        try:
            ...
            logger.error(f"Error in TikTok Scraper: {e}", exc_info=True)
...
def run_product_scraping():
    ...
    try:
        ...
        logger.error(f"Error in Product Scraper: {e}", exc_info=True)
