...
import { Send, Trash2, Bot, User, Loader2, AlertCircle, Plus, X } from 'lucide-react';
...
function Chat() {
  ...
  useEffect(() => {
    ...
    if (lastMsg.type === 'assistant') {
      ...
      // Auto-preview the first product link found
  ...
  // --- NEW: Handle Link Clicks (Interceptor) ---
  const handleMessageClick = (e) => {
    // Check if the clicked element is one of our product links
  ...
  const formatMessageContent = (content) => {
    ...
    // 1. Markdown Links -> Interceptor Links
    const markdownLinkRegex = /\[([^\]]+)\]\(([^)]+)\)/g;
    formatted = formatted.replace(markdownLinkRegex, (match, text, url) => {
      // Add 'product-link' class and data attribute to hijack the click
    ...
    // 4. Raw Links -> Standard External Links
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-slate-50 to-slate-100 overflow-hidden">
      <SEO
        ...
        description="Ask our AI Agent about SLT products, services, and enterprise knowledge."
      />
      ...
      <div className="flex flex-1 overflow-hidden relative">
        <div className="flex flex-col flex-1 relative min-w-0">
            ...
            <div className="max-w-5xl mx-auto px-4 py-6">
              {messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center py-12">
                  ...
                  <p className="text-slate-600 mb-8 max-w-md leading-relaxed">Ask me anything about SLT's products, services, or company information. I can help you with product prices, specifications, and more.</p>
                    ...
                    return (
                      <div key={message.id} className={`flex gap-3 ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        <div className={`flex flex-col max-w-3xl ${message.type === 'user' ? 'items-end' : 'items-start'}`}>
                          {/* --- MESSAGE BUBBLE CONTENT (With Click Handler) --- */}
          ...
          {error && (<div className="max-w-5xl mx-auto w-full px-4"><div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-4 shadow-sm"><div className="flex items-center gap-2"><AlertCircle className="w-5 h-5 text-red-600" /><p className="text-sm text-red-800 font-medium">{error}</p></div></div></div>)}
