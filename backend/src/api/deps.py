...
def get_optional_user(
    ...
    """
    Returns the Customer object if a valid token is present.
    Returns None if no token is provided (Guest User).
    """
    if not token:
        return None
    ...
    try:
        ...
        token_data = TokenData(email=payload.get("sub"), user_id=user_id)
    except JWTError:
        return None
